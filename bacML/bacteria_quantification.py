...
# python bacteria_quantification.py "C:\Users\10250153\bacteria3\experiment1" 40 2000 400 2000 1
# arguments:
# images_directory contains tif image stacks
# images_directory/img.tif
# images_directory/annot/img.tif
# images_directory/ml_region/train/img.tif
# images_directory/ml_region/validate/img.tif
# images_directory/ml_region/test/img.tif
...
def CNN2(num_classes, img_wdt, img_hgt, number_epochs, learning_rate):
    ...
    model = Sequential()
    model.add(Conv2D(256, (3, 3), input_shape=(img_hgt, img_wdt, 1), padding='same', activation='relu', kernel_constraint=maxnorm(3)))
...
for img in list_images:
    ...
    if os.path.isfile(annot_path):
        ...
        if not computed_train or recompute:
            ...
            ##########################################################################################################
            sample_idx = sample(1 - annot, mask_train, train_size)

            category, mapping = updatecat(bacteriaName + "_Neg", category, mapping)
            ...
            for idx in sample_idx:
                ...
                if (D / 2 <= x < width - D / 2) and (D / 2 <= y < height - D / 2):
                    if EXPORT_SVG:
                        f.write("<circle cx=\"%s\" cy=\"%s\" r=\"%s\" fill=\"blue\" fill-opacity=\"0.4\"/>\n" % (x + .5, y + .5, D / 2))
                    x_train.append(image_stack[length-1, int(x - D / 2):int(x + D / 2), int(y - D / 2):int(y + D / 2)]) # try 2d always take the last layer
            ...
            for idx in sample_idx:
                ...
                if (D / 2 <= x < width - D / 2) and (D / 2 <= y < height - D / 2):
                    ...
                    x_train.append(image_stack[length-1, int(x - D / 2):int(x + D / 2), int(y - D / 2):int(y + D / 2)])
                    y_train.append(category)
            ...
            ##########################################################################################################
            sample_idx = sample(1 - annot, mask_valid, valid_size)
            ...
            for idx in sample_idx:
                ...
                if (D / 2 <= x < width - D / 2) and (D / 2 <= y < height - D / 2):
                    if EXPORT_SVG:
                        f.write("<circle cx=\"%s\" cy=\"%s\" r=\"%s\" fill=\"blue\" fill-opacity=\"0.2\"/>\n" % (x + .5, y + .5, D / 2))
                    x_valid.append(image_stack[length-1, int(x - D / 2):int(x + D / 2), int(y - D / 2):int(y + D / 2)])
        ...
        if not computed_test or recompute:
            ...
            ##########################################################################################################
            # I_test.append(np.zeros((height, width), dtype=np.uint8))

            ##########################################################################################################
            sample_idx = sample(1 - annot, mask_test, test_size)
            ...
            for idx in sample_idx:
                ...
                if (D / 2 <= x < width - D / 2) and (D / 2 <= y < height - D / 2):
                    if EXPORT_SVG:
                        f.write("<circle cx=\"%s\" cy=\"%s\" r=\"%s\" fill=\"blue\" fill-opacity=\"0.6\"/>\n" % (x + .5, y + .5, D / 2))
                    x_test.append(image_stack[length-1, int(x - D / 2):int(x + D / 2), int(y - D / 2):int(y + D / 2)])
...
print("CNN2 nodel... nr classes = ", len(mapping), " ", X_train.shape)
model, epochs = CNN2(len(mapping), D, D, number_epochs, learning_rate)
print("created.")
